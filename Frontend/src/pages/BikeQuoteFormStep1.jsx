...
const baseURL = import.meta.env.VITE_BACKEND_URL;
 ...
 const handleClick = (id) => {
  ...
  useEffect(() => {
    const selectedBrand = dropdowns.find((d) => d.brand === brand);
    if (selectedBrand) {
  ...
  useEffect(() => {
    const selectedModel = modelOptions.find((m) => m.name === model);
    setYearOptions(selectedModel ? selectedModel.years : []);
  }, [model]);

const formatToWords = (num) => {
  if (!num || isNaN(num)) return "";
  if (num < 50000) return "Less than 50 Thousand";
  if (num >= 1000000) return "Maximum 10 Lakh reached"; // ✅ 10 lakh max
  ...
  return `${Math.floor(num / 1000)} Thousand`; // ✅ Between 50k and 1 lakh
};

const isValueValid = value >= 50000 && value <= 1000000;
 // ✅ 50k to 10 lakh
 // ✅ up to 10 crore
  ...
  const handleSubmit = (e) => {
    ...
    if (!isValueValid) {
      alert("Current value must be between 1 Lac and 1 Crore");
      return;
    }
    ...
    localStorage.setItem("bikeInfo", JSON.stringify(bikeData));
    navigate("/bike/step2");
  };
              ...
              <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 items-center gap-8 px-4">
                ...
                <div>
                  <h2 className="text-2xl md:text-3xl lg:text-5xl  font-mulish font-bold md:leading-[45px] lg:leading-[68px] leading-[40px] mt-20 md:mt-0 text-[#005190] mb-4">
                    Save Up To 30% On Your Bike Insurance In Minutes.
                  </h2>
            ...
            formElement.scrollIntoView({ behavior: "smooth" });
             ...
             className="flex items-center lg:gap-3 md:gap-2 bg-[#004E88] rounded-lg px-6 py-2 md:px-2 lg:px-6 text-left text-white hover:bg-[#003e6d] transition">
              ...
              <div>
                ...
                <p className="text-sm md:text-xs lg:text-sm text-white">Lowest Bike Premiums</p>
              </div>
            </button>
             ...
             className="flex items-center lg:gap-3 md:gap-2 bg-[#004E88] rounded-lg px-6 py-2 lg:px-6 md:px-2 text-left text-white hover:bg-[#003e6d] transition">
              ...
              <div>
                ...
                <div className="flex justify-center">
                  <img src={carImage} alt="Car" className="w-full max-w-xl" />
                </div>
              </div>
            </div>
  <div
...
>
  ...
  >
    ...
    <p className="text-center text-gray-500 mb-5 sm:mb-6 text-sm sm:text-base">
      Stay home & renew bike insurance in 2 minutes
    </p>
    ...
    >
      ...
      <div>
        <input
          type="number"
          placeholder="Enter value (min 50,000)"
          value={value}
          onChange={(e) => {
            let val = parseInt(e.target.value) || "";
            if (val > 1000000) val = 1000000; // Max 10 lakh
            setValue(val);
          }}
          required
          className="w-full border p-3 sm:p-3 rounded text-sm sm:text-base"
...
export default BikeQuoteFormStep1;
