...
const ComparePlans = () => {
  ...
  const carValue = searchParams.get("value");
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        //   `${baseURL}/api/plans/filter?value=${carValue}`
        // );
        const response = await fetch(`${baseURL}/api/plans/all`);
        const data = await response.json();
        // console.log("Plans fetched from backend:", data);
        setPlans(data.filter((plan) => plan.isEnabled));
      } catch (error) {
...
const handleBuyNow = (selectedPlan) => {
  setLoadingPlanId(selectedPlan._id); // Sirf clicked plan ka ID set karo

  const yearlyPremium = calculateYearlyPremium(Number(carValue), selectedPlan.rate);
  ...
  setTimeout(() => {
    setLoadingPlanId(null); // Loader band karo
    navigate("/schedule-survey");
  }, 3000);
};
...
const handleDownloadPDF = (plan) => {
  ...
  doc.setFontSize(18);
  doc.text("Car Insurance Plan", 20, 20);
  ...
  doc.text(
    `Yearly Premium: ${calculateYearlyPremium(Number(carValue), plan.rate)}`,
    20,
    70
  );

  doc.save(`${plan.name.replace(/\s+/g, "_")}-Plan.pdf`); // Spaces replaced with _
};
...
// const calculateTwoMonthPremium = (value, rateString) => {
//   const yearly = calculateYearlyPremium(value, rateString);
//   const twoMonth = (yearly / 12) * 2;
//   return twoMonth.toFixed(0);
// };
...
return (
  ...
  >
    Compare Car <br className="hidden sm:block" /> Insurance in Seconds
  </h2>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10 px-2 sm:px-4 lg:px-6">
      {plans.map((plan, index) => (
        ...
        >
          ...
          {/* Logo */}
          <div className="bg-transparent rounded-full mb-4 sm:mb-6">
            <img
              src={`${baseURL}/${plan.imageUrl}`}
              alt={plan.name}
              className="w-[120px] sm:w-[140px] lg:w-[150px] h-[50px] sm:h-[55px] lg:h-[81px] 
          ...
          {/* Rate */}
          <p className="text-sm sm:text-base flex justify-between text-[#8F8F8F] mt-3 mb-2 sm:mb-3 sm:mt-3 border-b border-[#8F8F8F] pb-1 sm:pb-2 w-full font-normal font-manrope">
            <span className="">Rate:</span> {plan.rate}%
          </p>
          ...
          <p className="text-sm sm:text-base flex justify-between text-[#8F8F8F] mt-3 sm:mt-3 mb-5 sm:mb-6 lg:mb-8 border-b border-[#8F8F8F] pb-1 sm:pb-2 w-full font-normal font-manrope">
            ...
            {calculateYearlyPremium(Number(carValue), plan.rate)}/Pkr
          </p>
  ...
  className=" border-1 border-[#005190] text-[#005190] 
             ...
             hover:text-white hover:bg-[#005190] hover:border-[#005190]
             active:text-white active:bg-[#005190] active:border-[#005190]"
...
>
  {loadingPlanId === plan._id ? (
    ...
    <p className="text-center text-gray-600 text-base sm:text-lg">No plans found.</p>
  )}
<button
  className="border border-[#005190] text-[#005190] 
             px-6 py-2 rounded 
             transition-all duration-300 
             font-medium text-base
             hover:bg-[#005190] hover:text-white"
>
  Test Button
</button>
